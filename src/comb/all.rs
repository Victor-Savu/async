...
pub struct ContinueRemaining<F, L>(PhantomData<(F, L)>) where F: Coroutine<Continue = F>, L: Coroutine<Yield = F::Yield, Continue = L>;

impl<F: Coroutine<Continue = F>, L: Coroutine<Yield = F::Yield, Continue = L>> ContinueRemaining<F, L> {
    fn new() -> Self {
        ContinueRemaining(PhantomData)
    }
}

impl<F: Coroutine<Continue = F>, L: Coroutine<Yield = F::Yield, Continue = L>> FnOnce<(Either<(F::Return, L), (F, L::Return)>,)> for ContinueRemaining<F, L> {
    type Output = Either<CoMapReturn<L, Prepend<F::Return>>, CoMapReturn<F, Append<L::Return>>>;

    extern "rust-call" fn call_once(self, (results, ): (Either<(F::Return, L), (F, L::Return)>,)) -> Self::Output {
        match results {
...
impl<F> All for F where F: Coroutine<Continue = F>{
    fn all<L>(self, l: L) -> CoAll<Self, L> where L: Coroutine<Yield = Self::Yield, Continue = L> {
